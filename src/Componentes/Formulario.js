...
export const Formulario = ({ setCategoria }) => { //Recibimos el props de nuestro contenedor padre

    const [inputValue, setInputValue] = useState(''); //Creamos nuestras variables de estado

    const accionEnter = (e) => { //Evento que se encarga al momento de enviar el formualrio con el dato
    ...
    const eventoTeclado = (e) =>{ //Evento cuando se dectecte algun cambio 
    ...
    return (
        <article>
            <form onSubmit={ accionEnter } noValidate autoComplete="off" className={classes.input}>
            <TextField id="outlined-basic" variant="filled" label="Buscador:"  onChange={ eventoTeclado} value={inputValue} size='medium' />
            </form>
        </article>
    )
}
